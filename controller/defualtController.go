...
func (dc *DefaultController) RunScript(script script.Scripter) error {
	...
	for _, cmd := range script.Commands {
		if true == cmd.IsComment {
			...
			if err != nil {
				if result == nil {
					result = make([]commons.ResultOutput, 0)
				}
				result := append(result, commons.NewCustomOutput("TIMEOUT", ""))
				dc.View.NotifyDisplay(&cmd, result, false)
			...
			// 通知view
			if nil != dc.View {
				dc.View.NotifyDisplay(&cmd, result, ok)
			}
			if false == ok {
				//执行失败, 不再执行后续指令
